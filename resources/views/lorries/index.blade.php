...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">{{ __('Vans') }}</li>
    </ol>
                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             {{ __('Vans') }}
                             @unless(auth()->check() && auth()->user()->isInventoryManager())
                             <a class="pull-right" href="{{ route('lorries.create') }}" title="{{ __('Create') }}" data-toggle="tooltip"><i class="fa fa-plus-square fa-lg"></i></a>
                             <a class="pull-right text-danger pr-2" id="massdelete" href="#" title="{{ __('Mass delete') }}" data-toggle="tooltip"><i class="fa fa-trash fa-lg"></i></a>
                             <a class="pull-right text-success pr-2" id="massactive" href="#" title="{{ __('Mass active') }}" data-toggle="tooltip"><i class="fa fa-check fa-lg"></i></a>
                             @endunless
                         </div>
                         <div class="card-body">
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/lorries/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/lorries/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/lorries/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status
