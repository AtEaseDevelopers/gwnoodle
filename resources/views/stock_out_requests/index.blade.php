...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">{{ $pageTitle ?? 'Stock-Out Approvals' }}</li>
    </ol>
    ...
    <!-- Reject Modal -->
    <div class="modal fade" id="rejectStockOutModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title"><i class="fa fa-times"></i> Reject Stock-Out Request</h5>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.approve-btn', function () {
                ...
                if (!confirm('Approve stock-out of ' + qty + ' unit(s) for batch ' + batch + '?\nThis will deduct the stock immediately.')) {
                    return;
                }
                ...
                $.ajax({
                    url: "{{ url('stockOutRequests') }}/" + id + "/approve",
                    type: "POST",
            ...
            $('#confirm-reject-btn').on('click', function () {
                ...
                $.ajax({
                    url: "{{ url('stockOutRequests') }}/" + id + "/reject",
                    type: "POST",
            ...
            $('#bulk-approve-btn').on('click', function () {
                ...
                if (!confirm('Approve ' + ids.length + ' selected stock-out request(s)?\nThis will deduct the stock immediately.')) {
                    return;
                }
                ...
                $.ajax({
                    url: "{{ route('stockOutRequests.bulk-approve') }}",
                    type: "POST",
            ...
            $('#confirm-bulk-reject-btn').on('click', function () {
                ...
                $.ajax({
                    url: "{{ route('stockOutRequests.bulk-reject') }}",
                    type: "POST",
            ...
            $('#confirm-edit-qty-btn').on('click', function () {
                ...
                $.ajax({
                    url: "{{ url('stockOutRequests') }}/" + id + "/update-qty",
                    type: "POST",
