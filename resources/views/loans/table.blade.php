...
@push('scripts')
    <script>
        $(document).on("click", ".PaymentDetails", function(e){
            getPaymentDetails($(this).attr("loanid"));
        });
        function getPaymentDetails(loanid){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/loanPayment/getPaymentDetails",
                type:"POST",
                data:{
                loanid: loanid
                ,_token: "{{ csrf_token() }}"
                ...
                success:function(response){
                    if(response == ''){
                        noti('i','Info','No payment details can be show...')
                        HideLoad();
                        return;
                    }
