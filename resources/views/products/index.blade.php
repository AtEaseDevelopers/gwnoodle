...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">{{ __('products.products')}}</li>
    </ol>
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/products/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        $(document).on("click", "#masssyncxero", function(e){
            ...
            $.confirm({
                ...
                buttons: {
                    Yes: function() {
                        let url = "{{config('app.url')}}/products/sync-xero"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/products/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/products/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status
