    ...
    <script>
        ...
        function check(id){
            $.ajax({
                ...
                url: "{{ config('app.url') }}/invoicePayments/getpayment/"+id,
                type: 'GET',
                cache: false,
                contentType: false,
                processData: false,
                success: function (response) {
                    if(response.status){
                        if(response.data.attachment != null){
                            var attachment = "{{ config('app.url') }}/"+response.data.attachment;
                        }else{
                            var attachment = "{{ config('app.url') }}/assets/img/invoicepayment/notfound.webp";
                        }
                        var amount = response.data.amount;
                        var action = "{{ config('app.url') }}/invoicePayments/updatepayment/"+id;
                        $('#check-modal').find('iframe').attr('src',attachment);
                        $('#check-modal').find('#approve_form').attr('action',action);
                        $('#check-modal').find('#approve_form').attr('action',action);
                        $('#check-modal').find('#approve_form').find('#amount').val(amount);
                        $('#check-modal').modal('show');
                    }else{
                        noti('e','Please contact your administrator',response.message);
                    }
                }
            });
        }
    ...
    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="infoModelLabel">{{ __('invoice_payments.select_a_date_range') }}</h5>
        ...
        <div class="modal-body">
            ...
            <div class="form-group col-sm-12">
                <label for="datefrommodel">{{ __('invoice_payments.date') }}:</label>
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-primary" onclick="dateRange();">{{ __('invoice_payments.search') }}</button>
          <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('invoice_payments.close') }}</button>
        </div>
      </div>
    </div>
</div>