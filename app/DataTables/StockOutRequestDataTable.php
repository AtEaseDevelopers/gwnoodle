...
class StockOutRequestDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ->addColumn('checkbox', function ($stockOutRequest) {
                // Only pending rows can be actioned in bulk, and only by an approver.
                if ($this->isApprover() && $stockOutRequest->isPending()) {
                    return '<input type="checkbox" class="checkboxselect" checkboxid="' . $stockOutRequest->id . '"/>';
                }
                return '';
            })
            ->addColumn('action', function ($stockOutRequest) {
                return view('stock_out_requests.datatables_actions', ['request' => $stockOutRequest])->render();
            })
            ->editColumn('created_at', function ($stockOutRequest) {
                return $stockOutRequest->created_at ? $stockOutRequest->created_at->format('d-m-Y H:i:s') : '-';
            })
            ->addColumn('product_name', function ($stockOutRequest) {
                return $stockOutRequest->product ? e($stockOutRequest->product->name) : '-';
            })
            ->addColumn('batch_code', function ($stockOutRequest) {
                return $stockOutRequest->batch ? e($stockOutRequest->batch->batch_code) : '-';
            })
            ->addColumn('warehouse_name', function ($stockOutRequest) {
                if ($stockOutRequest->warehouse) {
                    return '<span class="badge badge-info">' . e($stockOutRequest->warehouse->name) . '</span>';
                }
                return '<span class="text-muted">-</span>';
            })
            ->editColumn('quantity', function ($stockOutRequest) {
                $qty = '<span class="font-weight-bold">' . number_format($stockOutRequest->quantity) . '</span>';
                if ((int) $stockOutRequest->quantity !== (int) $stockOutRequest->requested_quantity) {
                    $qty .= ' <small class="text-muted"><del>' . number_format($stockOutRequest->requested_quantity) . '</del></small>';
                }
                return $qty;
            })
            ->editColumn('status', function ($stockOutRequest) {
                switch ((int) $stockOutRequest->status) {
                    case StockOutRequest::STATUS_APPROVED:
                        return '<span class="badge badge-success">Approved</span>';
                    case StockOutRequest::STATUS_REJECTED:
                        $badge = '<span class="badge badge-danger">Rejected</span>';
                        if ($stockOutRequest->approval_remark) {
                            $badge .= '<br><small class="text-muted">' . e($stockOutRequest->approval_remark) . '</small>';
                        }
                        return $badge;
                    default:
     ...
     *
     * @param \App\Models\StockOutRequest $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(StockOutRequest $model)
    {
        return $model->newQuery()
            ...
            ->select('stock_out_requests.*')
            ->orderBy('stock_out_requests.created_at', 'desc');
    }
    ...
    {
        return 'stock_out_requests_' . date('Y-m-d_H-i-s');
    }
}
